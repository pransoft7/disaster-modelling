...
plt.tight_layout()
plt.savefig(os.path.join(root_dir, 'feature_importance_rf.png'))

# Function to process raster in chunks to avoid memory issues
...
susceptibility_map = process_raster_chunks(aligned_rasters, rf_model, scaler)

# Save the susceptibility map
output_file = os.path.join(root_dir, 'landslide_susceptibility_rf.tif')
...
print(f"Susceptibility map saved to {output_file}")
