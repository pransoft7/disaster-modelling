...
# Generate non-landslide points
non_landslide_points = gpd.GeoDataFrame(
    geometry=[Point(x, y) for x, y in zip(
        np.random.uniform(reference_profile['transform'][2], reference_profile['transform'][2] + reference_profile['width'] * reference_profile['transform'][0], len(landslides)),
        np.random.uniform(reference_profile['transform'][5] + reference_profile['height'] * reference_profile['transform'][4], reference_profile['transform'][5], len(landslides))
    )],
    crs=reference_profile['crs']
)
...
plt.tight_layout()
plt.savefig(os.path.join(root_dir, 'feature_importance_knn.png'))
...
susceptibility_map = process_raster_chunks(aligned_rasters, knn_model, scaler)

# Save the susceptibility map
output_file = os.path.join(root_dir, 'landslide_susceptibility_knn.tif')
...
plt.title('Landslide Susceptibility Map (KNN)')
plt.axis('off')
plt.tight_layout()
plt.savefig(os.path.join(root_dir, 'landslide_susceptibility_map_knn.png'), dpi=300, bbox_inches='tight')
plt.close()
