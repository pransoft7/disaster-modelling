...
susceptibility_map = process_raster_chunks(aligned_rasters, xgb_model, scaler)

# Save the susceptibility map
output_file = os.path.join(root_dir, 'landslide_susceptibility1.tif')
...
plt.axis('off')
plt.tight_layout()
plt.savefig(os.path.join(root_dir, 'landslide_susceptibility_map.png'), dpi=300, bbox_inches='tight')
plt.close()
