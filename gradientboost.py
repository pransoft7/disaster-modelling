...
susceptibility_map = process_raster_chunks(aligned_rasters, gb_model, scaler)

# Save the susceptibility map
output_file = os.path.join(root_dir, 'landslide_susceptibility_gb.tif')
...
plt.axis('off')
plt.tight_layout()
plt.savefig(os.path.join(root_dir, 'landslide_susceptibility_map_gb.png'), dpi=300, bbox_inches='tight')
plt.close()
